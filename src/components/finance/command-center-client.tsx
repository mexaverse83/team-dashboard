...
import {
  ...
  ArrowRight, Plus, Activity, Sparkles, ShieldAlert,
} from 'lucide-react'
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <GlassCard>
              <SectionHeader
                title="Budget pace"
                subtitle="On-track vs projected month-end"
                action={{ label: 'All budgets', href: '/finance/budgets' }}
              />
              {summary && summary.current_month.budget_vs_actual.length > 0 ? (
                <div className="space-y-3">
                  {summary.current_month.budget_vs_actual.slice(0, 5).map(cat => {
                    ...
                    const barColor = cat.status === 'over' ? 'bg-rose-500' : cat.status === 'warning' ? 'bg-amber-500' : 'bg-emerald-500'
                    return (
                      <div key={cat.category}>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm font-medium">{cat.icon} {cat.category}{cat.is_non_monthly && <span className="text-blue-400 text-[10px] ml-1">({cat.cycle_months}mo cycle)</span>}</span>
